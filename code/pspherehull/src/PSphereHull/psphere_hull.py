...
class PSphereHull:
    ...
    def contains(self, new_data, hypercube_filter=False, use_pcylinders=True, ):
        """
        ...
        for ivec in range(data.shape[0]):
            # Optionally assign False if the vector is outside the data hypercube
            if hypercube_filter and (not point_in_hypercube(data[ivec, :], self.hc_min, self.hc_max)):
...
#            return containing_spheres
    ...
    def make_volume_ratios(self, n_sample=0, hypercube_filter=False, use_pcylinders=True):
        """
        ...
        if n_sample > 0:
            ...
            n_inside = len(np.where(self.contains(sample, hypercube_filter, use_pcylinders))[0])
            if n_inside > 0:
                if use_pcylinders:
