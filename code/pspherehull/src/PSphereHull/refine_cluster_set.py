...
def refine_linear_clusters(
    data, labels, centers, min_size=0, pct_threshold=0.9, verbose=False
):
    """
    Purpose
    -------
    ...
    if data.shape[0] != len(labels):
        print("refine_cluster_set: data and labels have incompatible dimensions")
        return labels, centers
    if (centers.shape[0] - 1) != np.max(labels):
        print("refine_cluster_set: number of centroids is incompatible with label set")
        return labels, centers
    if pct_threshold < 0 or pct_threshold > 1:
        print("refine_cluster_set: pct_threshold must be between 0 and 1")
        return labels, centers
    # Start processing
    fixed = []
    ...
    while continue_splitting:
        ...
        # If remove_outlier has been used before calling this function, ignore the value -1.
